...
if (config.pwa && config.pwa.enable) {
    const {pwa} = config;
    baseWebpackConfig.entry.app.push(
        `${resolvePath('plugins/sw/client.js')}?flush=${pwa.flush}&scope=${pwa.scope || config.urlPrefix || '/'}&filename=${config.urlPrefix}${config.assertPath}/${pwa.filename}`
    );
}
