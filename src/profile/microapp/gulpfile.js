...
const {PLUGIN_MINIPROGRAM_ROOT, PLUGIN_ROOT, TYPE_MAPPER} = require('./consts')
...
async function toUrl(defUrl, file, imageDomain) {
    ...
    let url = path.resolve(file.dirname, defUrl)
    const parsedUrl = parseUrl(url) // 格式化成为对象的路径
    // 带有 inline  的就嵌入，不带的就
    if (parsedUrl.query && parsedUrl.query === '__inline') {
        ...
        const extname = path.extname(pathname).toLocaleLowerCase()
        switch (true) {
    ...
    const cwd = normalizeUrl(process.cwd())
...
function translateUrl() {
    return through2.obj((file, _, cb) => {
        ...
        if (file.isBuffer()) {
            ...
            const queue = []
            content.replace(/url\s*\(\s*['|"]?([^'")]+)['|"]?\s*\)/gim, (match, url) => {
            ...
            Promise.all(queue)
                .then(data => {
                    content = content.replace(/url\s*\(\s*['|"]?([^'")]+)['|"]?\s*\)/gim, (match, url) => match.replace(/url\s*\(\s*['|"]?([^'")]+)['|"]?\s*\)/, 'url(' + `${data.shift()}` + ')'))
...
function translateLessWithTheme(theme, themeData) {
    function addRuleWithPrefix(rules) {
        const prefix = `.${config.themePrefix}${theme}`
        // 获取__ROOT__ 标识
        // 遍历所有 selector
        // 匹配
        const roots = []
        rules.forEach(rule => {
            ...
            const isRoot = rule.declarations.filter(declaration => declaration.type === 'comment' && declaration.comment.indexOf('__ROOT__') !== -1)[0]
            if (isRoot) {
                roots.push(...rule.selectors)
            }
        })
        if (roots && roots.length) {
            const reg = new RegExp(`^(${roots.join('|').replace(/\./g, '\\.')})(\$|\\s+)`)
            rules.forEach(rule => {
                if (!rule.selectors) {
                    // keyFrams
                    return
                ...
                rule.selectors = rule.selectors.map(selector => {
                    if (reg.test(selector)) {
                        ...
                        const selectorArr = selector.split(/\s+/)
    ...
    function extFileExists(file) {
        ...
        let matched = false
        let extFile = resolvePath(file.history[0]).replace(/\/(pages|components)\//, (word, type) => {
            matched = true
                ...
                if (file.isBuffer()) {
                    try {
                        if (config.enableTheme || (config.globalComponents && config.globalComponents.enable)) {
                            ...
                            const tree = htmlparser2.parseDOM(content, {
                                xmlMode: true,
                                decodeEntities: true
                            })

                            if (config.enableTheme) {
                                const parentNodes = tree.filter(element => !element.parent)
                            ...
                            if (config.globalComponents && config.globalComponents.enable) {
                                if (isFunction(config.globalComponents.injection)) {
                                    const data = config.globalComponents.injection(file)
                                    if (data) {
                                        tree.push({
                                            data,
                                            type: 'text'
                            ...
                            const html = htmlparser2.DomUtils.getOuterHTML(tree, {
                                xmlMode: true,
            ...
            through2.obj((file, _, cb) => {
                if (file.isBuffer()) {
                    try {
                        ...
                        const re = resolveComponent(content, file, {
                            commonPageConfig,
                            root: config.root,
                            platform: config.platform,
                        ...
                        const stream = createStreamFromFile(newFile)
                        stream
                            .pipe(
                                rename(path => {
                                    replacePrefix(path)
...
function generateApp() {
    const router = []
    const subPackages = []
    const isPlugin = config.cmdArgv.plugin
    // 根据编译后的产出目录中的 pages 目录下的文件，自动生成 app.json 中的 pages 配置
    let distProjectPath = path.join(config.root, DIST_PROJECT_PATH)
    let distPagesPath = ''
    // 插件
    if (isPlugin) {
    ...
    genSubPackages(distProjectPath, subPackages, config.platform, filepath => {
    ...
    const outputPath = path.join(config.projectPath, appPath)
            ...
            through2.obj((file, _, cb) => {
                if (file.isBuffer()) {
                    ...
                    try {
                        ...
                        const plugins = Object.keys(content.plugins)
                        ...
                        if (config.globalComponents && config.globalComponents.enable) {
                            ...
                            Object.keys(config.globalComponents.components).forEach(key => {
...
function generatePluginJSON() {
    const router = []
    // 根据编译后的产出目录中的 pages 目录下的文件，自动生成 app.json 中的 pages 配置
    const distProjectPath = path.join(config.root, DIST_PROJECT_PATH) // dist/wechat/sandbox/project
    const distPagesPath = path.join(distProjectPath, path.relative(config.root, config.pluginPagesPath)) // dist/wechat/sandbox/project/plugin/pages
    if (fs.existsSync(distPagesPath)) {
    ...
    const outputPath = path.join(config.projectPath, PLUGIN_ROOT)
            ...
            through2.obj((file, _, cb) => {
                if (file.isBuffer()) {
                    ...
                    try {
                        ...
                        const pagesObj = genPluginPages(router)
                        switch (config.platform) {
                            case 'wechat':
                                content.pages = content.pages || {}
...
const del = function (file, src = '', dist = '') {
    const filePathFromSrc = path.relative(path.join(process.cwd(), src), file)
    const destFilePath = path.resolve(path.join(DIST_PROJECT_PATH, dist), filePathFromSrc)
    rm(destFilePath)
}

const watchHandler = function (event, file) {
    const extname = path.extname(file)
    const isModify = event === 'add' || event === 'changed'
    switch (true) {
        case file === 'app.json':
            if (isModify) {
...
const defaultTask = function () {
    ...
    const defaultTasks = [parallel(sync, translateLess, translateTpl, translateJs, translateImage), generateApp]
    ...
    if (config.cmdArgv.theme && config.enableTheme) {
        defaultTasks.push(
            ...config.themes.map(theme =>
                // 颜色名及其配置
                translateLessWithTheme(theme, config.themeData[theme])
            )
        )
    }
