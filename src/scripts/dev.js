...
if (config.enableDll) {
    ...
    // 调用一次 webpack
    const compiler = webpack(dllConf);
    // 开始读取 record 之前调用
    compiler.run(err => {
        if (!err) {
            devServe();
...
function devServe() {
    ...
    utils.getPort().then(port => {
        ...
        // const wdmLogger = compiler.getInfrastructureLogger('webpack-dev-middleware');
        const app = connect();
        if (devServerConf.hot) {
        ...
        const wdmInstance = webpackMiddleware(compiler, {
