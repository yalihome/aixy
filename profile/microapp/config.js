...
let { PLUGIN_ROOT, PLUGIN_MINIPROGRAM_ROOT, ENV_MAPPER, MAPPER } = require('./consts');
...
exports.init = function (Config) {
    class ProfileConfig extends Config {
        constructor() {
            ...
            var imageDomain = '';
            var conf = {};
            let isPlugin = this.config.cmdArgv.plugin;
            let confPath = new Array();
            ...
            function mergeConf(pathname) {
                let tp = path.resolve(pathname, MAPPER[ENV]);
                if (fs.existsSync(tp)) {
        ...
        resolveArgs(cmd) {
            switch (cmd) {
                case 'pack':
                    ...
                    console.log('pack:');
                    console.log(`${[name, this.config.platform, this.config.env, `v${version}`, ts].join('-')}.zip`);
                    return {
